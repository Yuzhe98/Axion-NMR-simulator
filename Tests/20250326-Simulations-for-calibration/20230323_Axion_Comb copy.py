...
from DataAnalysis import Exclusion, LIASignal
...
precessionsignature = signal.square(2 * np.pi * (1./period) * timestamp, duty=dutycycle)
timestamp_onduty = np.linspace(start=0, stop=dutycycle * period, num=dutylen)
timestamp_offduty = np.linspace(start=0, stop=(1-dutycycle) * period, num=periodlen - dutylen)
...
Omgea_a = 1
NMRenvelope = Omgea_a * T2star * (1-np.exp(-timestamp_onduty/T2star)) * np.exp(-timestamp_onduty/T2star) 
...
for i in range(numperiod):
    phase0 = phase_list[i]
    st_x = np.append(st_x, NMRenvelope * np.cos(2*np.pi*nu_rot*timestamp_onduty+phase0))
    st_y = np.append(st_y, NMRenvelope * np.sin(2*np.pi*nu_rot*timestamp_onduty+phase0))
