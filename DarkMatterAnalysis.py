...
from functioncache import (Lorentzian, MovAvgByStep, ReturnMeasureTimFromFile,
                           ReturnNMRdata, ReturnT2time, axion_lineshape, 
                           PlotGaussianLine, angle_between, record_runtime_YorN, GetDateTimeSimple,
                           merge_close_entries2, find_matching_tuples, 
                           read_double_precision_floats, 
                           check, extract_number, 
                           clear_lines, print_progress_bar, get_random_color)
...
class dmScanStep:
            ...
            for singlefile in file_list: 
                ...
                if singlefile[-3:] == '.h5':
                    with h5py.File(singlefile, 'r', driver='core') as dataFile:  # h5py loading method
                        ...
                        if verbose:
                            check(dataFile.keys())
                        if f'{device_id:s}/demods/{demod_index:d}/samplex' in dataFile.keys():
                            recordmethod = 'DAQ_record'
                        elif f'000/{device_id:s}/demods/{demod_index:d}/sample/x' in dataFile.keys():
                            recordmethod = 'UI'
                        elif f'000/{device_id:s}/demods/{demod_index:d}/sample.x/value' in dataFile.keys():
                            recordmethod = 'DAQ_continuous'
                        elif 'NMRKineticSimu/demods/0' in dataFile.keys():
                            recordmethod = 'NMRKineticSimu'
                        else:
                            raise ValueError('LoadMeasInfo() cannot figure out the recording method')
                        if verbose:
                            print('recordmethod: ', recordmethod)
                        
                        if recordmethod == 'DAQ_record':
                            Stream.dmodfreq = dataFile[f'{device_id:s}/demods/{demod_index:d}/dmodfreq'][0]
                            Stream.samprate = dataFile[f'{device_id:s}/demods/{demod_index:d}/samprate'][0]
                            Stream.data_len += len(list(dataFile[f'{device_id:s}/demods/{demod_index:d}/samplex']))
                        elif recordmethod == 'UI':
                            Stream.dmodfreq = dataFile[f'000/{device_id:s}/demods/{demod_index:d}/sample/frequency'][0]
                            Stream.samprate = dataFile[f'000/{device_id:s}/demods/{demod_index:d}/rate/value'][0]
                        elif recordmethod == 'DAQ_continuous':
                            pass
                        elif recordmethod == 'NMRKineticSimu':
                            Stream.dmodfreq = dataFile['NMRKineticSimu/demods/0/dmodfreq'][0]
                            Stream.samprate = dataFile['NMRKineticSimu/demods/0/samprate'][0]
                        else:
                    ...
                    Stream.acq_arr = np.array([[0, len(Stream.timestamp)],[0, len(Stream.timestamp)]], dtype=np.int64)
                else:
    ...
    def InsertFakeAxion(
        ...
        f0 = nu_a_index
        f1 = min(len(self.nps.frequencies) - 1, f0 + int(self.axion.lw_p * 10.))
        freq = self.nps.frequencies[f0:f1]
        # check(freq.shape)
        # check((self.nps.avgFFT[f0:f1]).shape)
        self.nps.avgFFT[f0:f1] += g_a * \
            self.axion.GetFFTsignal(
                freq=freq,
                rand_amp=rand_amp,
    ...
    def MovAvgByStep(
        ...
        weights = self.axion.lineshape_t(\
                self.axion.nu_a + np.arange(0., 10 * self.axion.lw_Hz, step=self.nps.freq_resol))
        self.nps.psdMovAvgByStep(
            weights=weights,
            step_len=step_len,
            verbose=False)
        self.axion.lw_p = int(np.ceil(self.axion.lw_Hz / self.nps.freq_resol))
    ...
    def Hz2Index(
        ...
        return lenofdata // 2 + int(np.ceil((freq_Hz - center_freq) / self.nps.freq_resol))
    ...
    def MakeAnalysisWindow(
        ...
        halfwidth_p = int(np.ceil(self.DS_lw_p * awin_width_in_DS_lw / 2.))
        
        # self.resnt_region_in_Hz = [self.spec_center_index - halfwidth_in_pixel, \
        #     self.spec_center_index + halfwidth_in_pixel]
        self.resnt_region_index = [self.spec_center_index - halfwidth_p, \
    ...
    def GetNoPulsePSD(
        ...
        self.nps.avgPSD = np.abs(self.nps.avgFFT) ** 2.
    ...
    def amp_ga_1iter(
        ...
        step.nps.CreateArtificialStream(
            dmodfreq=dmodfreq, samprate=samprate, total_dur=total_dur,
            year=year, month=month, day=day, time_hms=time_hms,  # Use UTC time!
            )
        ...
        step.nps.avgPSD = np.abs(step.nps.avgFFT) ** 2.
        # step.nps.GetSpectrum(showtimedomain=False, showfreqdomain=True, spectype='PSD')
        if use_sg:
            step.nps.avgPSD = step.nps.sgFilterPSD(\
                window_length=step.axion.lw_p // sg_axlw_frac, polyorder=sg_order, makeplot=False)
        # step.nps.GetSpectrum(showtimedomain=False, showfreqdomain=True, spectype='PSD')
        step.MovAvgByStep(
            step_len=max(1, step.axion.lw_p // 5),
            verbose=False)
        # step.nps.GetSpectrum(showtimedomain=False, showfreqdomain=True, spectype='PSD')
        r1 = step.nps.Hz2Index(step.axion.nu_a - 5.*step.axion.lw_Hz)
        # check(r1)
    ...
    def FindCandidate(
        ...
        NonNegative_arr = np.where(self.nps.avgPSD[self.r0:self.r1] - threshold < 0, \
                                0, self.nps.avgPSD[self.r0:self.r1] - threshold)
        
        peaks, properties = find_peaks(self.nps.avgPSD[self.r0:self.r1], height=threshold, \
                    distance=1, prominence=threshold, width=1)
        ...
        if makeplot:
            ...
            NoPulsePSD_ax.plot(self.Index2Hz(np.arange(self.r0, self.r1)), \
                self.nps.avgPSD[self.r0:self.r1], color='tab:blue', \
                               label='NoPulse PSD after MovAvg')
            NoPulsePSD_ax.set_ylabel('PSD')
            NonNegative_ax.plot(NonNegative_arr, color='tab:green', label='PSD > threshold * std')
            NoPulsePSD_ax.set_ylabel('PSD')
            ...
            CND_ax.plot(self.Index2Hz(np.arange(self.r0, self.r1)), \
                        self.nps.avgPSD[self.r0:self.r1], color='tab:blue', label='NoPulse PSD after MovAvg')
            CND_ax.plot(index2hz_r0offset(peaks), self.nps.avgPSD[self.r0 + peaks], \
                        "x", color='tab:orange', label='Candidate(s)')
            
            # draw candidate(s)' height and width
            CND_ax.vlines(x=index2hz_r0offset(peaks) , ymin=self.nps.avgPSD[self.r0 + peaks] - properties["prominences"], \
                ymax = self.nps.avgPSD[self.r0 + peaks], color = "tab:orange")
            CND_ax.hlines(y=properties["width_heights"], \
                          xmin=index2hz_r0offset(properties["left_ips"]),
                xmax=index2hz_r0offset(properties["right_ips"]), color = "tab:orange")
            
            # draw the threshold
            CND_ax.hlines(y=threshold, xmin=NoPulsePSD_ax.get_xlim()[0],
                xmax=NoPulsePSD_ax.get_xlim()[1], color = "tab:grey", linestyles='--', label='threshold')
            
            numofbin_input = int(3 * np.ceil(np.ptp(self.nps.avgPSD[self.r0:self.r1]) / std))
            hist, bin_edges = np.histogram(self.nps.avgPSD[self.r0:self.r1], \
                                           bins=max(numofbin_input, 6), density=False)
            ...
            for i, count in enumerate(hist):
                if count > 0:
                    ...
                    hist_y.append(count)
            hist_ax.scatter(hist_x, hist_y, \
                             color='green', edgecolors='k', linewidths=1, marker='o', s=6, zorder=6,
                                label=f'histogram')
            hist_ax.vlines(x=threshold, ymin=0.1, \
                ymax = np.amax(hist_y), color = "tab:grey", label='threshold', linestyles='--')
...
class dmScan:
    def __init__(self,
        ...
        self.analysisname=analysisname # name of the folder where the axionfinder results are saved
        self.scanInfo={}
        self.allDMfiles=[]
        self.allNMRfiles=[]
        self.allCPMGfiles=[]
        ...
        self.pcnvCND=[] # convolved PSD values at the candidate frequencies # pcnvCND=(10^powerOfTenScaling)*convolvedPSD(candind)
        self.prawCND=[] # raw PSD values at the candidate frequencies # prawCND=PSDwindowNoBuffer(psdcandind)
        self.gCND=[] # coupling strength of the candidate axions
        self.threshold_rescan=[] # the PSD value at the rescan threshold # pLIM=(10^powerOfTenScaling)*candThreshVal
        self.threshold_exclude=[] # the convolved PSD value at the exclusion threshold # SNRtarget=(10^powerOfTenScaling)*excThreshVal
        ...
        self.cands_filtered=[] # ntuples of cands after various steps of filtering done in FilterCandidates()
    ...
    def LoadScanInfo(
        ...
        for subscan in self.name:
            if "20221223" in subscan:
                ...
                MATfiles = glob.glob(rf"{self.basepath}/{subscan}/AxionFinder results/{self.analysisname}/*/*vars.mat")
            elif "20221214" in subscan:
                #print("a scan from day 1")
                DMfiles = glob.glob(rf"{self.basepath}/{subscan}/0Pulse data/0Pulse_*/stream_*.h5")
                NMRfiles = glob.glob(rf"{self.basepath}/{subscan}/1Pulse data/1Pulse_*/stream_*.h5")
                CPMGfiles = glob.glob(rf"{self.basepath}/{subscan}/CPMG data/*/CPMG_data.h5")
    ...
    def MakeDataTextfiles(
        ...
        if scannames == None: scannames = ["20221223_154712","20221223_175006","20221223_194651"]
        ...
        for scanindex, scanfiles in enumerate(scans.allDMfiles):
            ...
            for runindex, runfile in enumerate(scans.allDMfiles[scanindex]):      
                nu_n, FWHM_n, amp_n, nu_n_err, FWHM_n_err, amp_n_err = ReturnNMRdata(basepath, scannames[scanindex], runindex)
                year, month, day, time_str, datetime_int = ReturnMeasureTimFromFile(runfile)#2022, 12, 23, '15:47:18'
                ALPparams = AxionWind().GetALP_Data(year=year,month=month,day=day,time_str=time_str,datetime_int=datetime_int,
                                                        T_acq=0.5,T2time=ReturnT2time(basepath, scannames[scanindex]),
                                                        NMRfreq=[nu_n,nu_n_err],
                                                        NMRwidth=[FWHM_n,FWHM_n_err],
                                                        NMRamp=[amp_n,amp_n_err],
                                                        nu_ALP=0, g_aNN=1,
                                                        usedailymod=True)[:-1] # don't load uncertainties                     
                allALPparams.append(ALPparams)
                print_progress_bar(runindex, total=len(scans.allDMfiles[scanindex]), prefix='Progress', suffix='Complete', length=50)
                #NMRfluxes.append(ALPparams[8])
                taus.append(ALPparams[3])
            print(f"average tau: {np.mean(taus)}")        
            np.savetxt(f'{basepath}/{scannames[scanindex]}/_DM_measurements_data.txt', allALPparams)
    ...
    def tsCheckSanity(
        ...
        for i, step in enumerate(self.step_list):
            print_progress_bar(i, total, length=50, prefix='Checking sanity of scan steps')
            step.nps.LoadStream(verbose=verbose)
            report = step.nps.tsCheckSanity(
                plotIfInsane=plotIfInsane, 
                verbose=verbose)
        ...
        print_progress_bar(total, total, length=50, prefix='Checking sanity of scan steps')
    ...
    def FindCandidate(
        ...
        for scanindex, scan in enumerate(scans):
            if verbose: check(self.allDMfiles[scanindex])
            check(scan)

            measdatafile = f'{self.basepath}/{scan}/_DM_measurements_data.txt'
            larmors = np.genfromtxt(measdatafile, unpack=True, delimiter=' ', skip_header=0, filling_values=0, invalid_raise=False)[5]
            all_iCND=[]
            all_fCND=[]
            all_gCND=[]
            all_pcnvCND=[]
            ...
            for runind, file in enumerate(self.allDMfiles[scanindex]):
                ...
                if self.singlerunfile is not None: 
                    ...
                    if verbose: print("looking for singlerunfile")
                else: 
                    matlogfile = rf'{self.basepath}/{scan}/AxionFinder results/{self.analysisname}/{runind+1}/{self.analysisname}_vars.mat'
                    if verbose: print("looking for this run's matfile")
                mat_data = loadmat(matlogfile)
                #mat_dict = convert_matlab_cells(mat_data)

                self.powerOfTenScaling = mat_data['powerOfTenScaling'].flatten().tolist()[0] # scaling factor
                
                if howtothresh == 0 or howtothresh == 1:       
                    fCND = mat_data['fCND'].flatten().tolist() # candidate frequencies
                    prawCND = mat_data['prawCND'].flatten().tolist() # raw PSD values at the candidate frequencies # prawCND=PSDwindowNoBuffer(psdcandind)
                    pcnvCND = mat_data['pcCND'].flatten().tolist() # convolved PSD values at the candidate frequencies # pcnvCND=(10^powerOfTenScaling)*convolvedPSD(candind)
                    pLIM = mat_data['pLIM'].flatten().tolist()[0] # the PSD value at the rescan threshold # pLIM=(10^powerOfTenScaling)*candThreshVal
                    threshold_rescan = pLIM / (10**self.powerOfTenScaling) # the NPE plot value at the rescan threshold
                    # the convolved PSD value at the exclusion threshold # pEXC=(10^powerOfTenScaling)*excThreshVal
                    try: SNRtarget = mat_data['SNRtarget'].flatten().tolist()[0]  
                    except KeyError: SNRtarget = mat_data['pEXC'].flatten().tolist()[0]   
                    threshold_exclude = SNRtarget / (10**self.powerOfTenScaling) # the NPE plot value at the exclusion threshold
                    try: iCND = mat_data['candind'].flatten().tolist()    
                    except KeyError:
                        #convolvedFreq = mat_data['convolvedFreq'].flatten().tolist() # frequencies array of convolved PSD
                        convolvedPeak = mat_data['convolvedPeak'].flatten().tolist() # convolved PSD values array
                        iCND = [index for index, x in enumerate(convolvedPeak) if convolvedPeak[index] > threshold_rescan]
                ...
                # normalFitInfo contains:
                    ...
                    fitparams = [normalFitInfo[11],normalFitInfo[1],normalFitInfo[2],normalFitInfo[3]]
                    fitcurve = fitparams[0]*fitparams[1]*norm.pdf(bincenters,fitparams[2],fitparams[3])
                    ...
                    fitparams = [chi2FitInfo[1],chi2FitInfo[2],chi2FitInfo[3],chi2FitInfo[4]]
                    fitcurve = chi_squared_pdf(bincenters, fitparams)
                    threshold_rescan = chi2FitInfo[15] # this threshold is probably incorrect. it was calculated still using the gaussian params
                    threshold_exclude = 5.0*chi2FitInfo[4] # 5 sigma          

                elif howtothresh == 4:  
                    v0 = [15, 0.01, -50, 20] # this worked OK as a guess for fitting in matlab
                    v0 = [405.16583457911196, -0.0009256657609884802, 15363.066801881694, 15376.542179937464] # result of fit in matlab
                    bounds = [(1e2, 1e8),   # prefactor
                            (-1e5, 1e5),   # scale
                            (-1e5, 1e5), # shift
                            (2, 1e2)] # dof # should not be much more than 10 I guess
                    result = minimize(RNCF_chi2, 
                        x0=v0, 
                        #bounds=bounds,
                        method='Nelder-Mead',
                                    )
                    ...
                    p95 = chi2.ppf(1 - 0.95, fitparams[3]) # the 95 percentile for a chi-square curve with dof as found by our fit
                    ...
                    p95 = gamma.ppf(1-0.95, fitparams[3], loc=fitparams[4]+6.0*np.sqrt(fitparams[3])*fitparams[5], scale=fitparams[5])
                    threshold_rescan = fitparams[1] * (p95 - fitparams[2])
                ...
                if howtothresh > 0: # coupling values have to be recalculated for the candidates found
                    
                    if howtothresh > 1:
                        convolvedFreq = mat_data['convolvedFreq'].flatten().tolist() # frequencies array of convolved PSD
                        convolvedPeak = mat_data['convolvedPeak'].flatten().tolist() # convolved PSD values array
                        if verbose: check(convolvedFreq)
                        if verbose: check(convolvedPeak)

                        iCND = [index for index, p in enumerate(convolvedPeak) if convolvedPeak[index] > threshold_rescan]
                        fCND = [f for index, f in enumerate(convolvedFreq) if convolvedPeak[index] > threshold_rescan]
                        pcnvCND = [p*(10**self.powerOfTenScaling) for index, p in enumerate(convolvedPeak) if convolvedPeak[index] > threshold_rescan]
                    ...
                    gCND = []
                    nu_n, FWHM_n, amp_n, nu_n_err, FWHM_n_err, amp_n_err = ReturnNMRdata(self.basepath, scan, runind)
                    year, month, day, time_str, datetime_int = ReturnMeasureTimFromFile(file)#2022, 12, 23, '15:47:18'
                    for index, i in enumerate(fCND): # calculate ALP signal for each candidate frequency
                        fakeAxion_Amp = AxionWind().GetALP_Data(year=year,month=month,day=day,time_str=time_str,datetime_int=datetime_int,
                                                                T_acq=0.5,T2time=ReturnT2time(self.basepath, scan),
                                                                NMRfreq=[nu_n,nu_n_err],
                                                                NMRwidth=[FWHM_n,FWHM_n_err],
                                                                NMRamp=[amp_n,amp_n_err],
                                                                nu_ALP=i, g_aNN=1)[9]             
                    ...
                    gCND = mat_data['gCND'].flatten().tolist() # coupling strength of the candidate axions
                ...
                if verbose: print(f'{len(fCND)} outliers found in file: {matlogfile}')
                ...
                for candindex, cand in enumerate(fCND):
                    all_candidates.append([scan, runind, fCND[candindex], gCND[candindex]])
                ...
                if plot_opt:
                    ...
                    firstnonzero_ind = next((i for i, x in enumerate(convhist) if x != 0), len(convhist))
                    ...
                    if normalizebysigma: bincenters = bincenters / np.std(convolvedPeak)

                    plt.figure(figsize=(16*0.9, 9*0.9), dpi=100)
                    plt.scatter(bincenters, counts, s=100, marker='o', label='Filtered PSD', alpha=0.9)#color='blue'

                    try:
                        check(fitparams)
                        plt.plot(bincenters, fitcurve, color='green', linewidth=4, label=fitlabel, alpha=0.9 )
                    except NameError: pass 
                    
                    plt.axvline(x=threshold_rescan, color='green', linestyle='--', linewidth=4, alpha=0.7, label=rf'Detection threshold ($95~\%$ confidence)')
                    plt.axvline(x=threshold_exclude, color='red', linestyle='--', linewidth=4, alpha=0.7, label=rf'Sensitivity threshold ($5\sigma$)')

                    outlier_inds = [index for index, x in enumerate(bincenters) if x > threshold_rescan]
                    plt.scatter([bincenters[i] for i in outlier_inds], [counts[i] for i in outlier_inds], s=100, color='green', marker='o', label='Signal candidates', alpha=0.9)
                    ...
                    #plt.autoscale()
                    plt.tight_layout()
                    plt.savefig(rf'{savepath}/convhistplot_{scan}_{runind}.png', bbox_inches='tight')
                    #plt.show()
                    plt.clf()
                    plt.close()

                print_progress_bar(runind, total=len(self.allDMfiles[scanindex]), prefix='Progress', suffix='Complete ', length=50)

            num_outliers = sum(len(cands) for cands in all_fCND)
            print(f'total # outliers in all runs: {num_outliers}')
            print(f'avg # outliers per run: {num_outliers / len(self.allDMfiles[scanindex])}')
    ...
    def FilterCandidates(
        ...
        for scanindex, scan in enumerate(scans):
            check(scan)
            measdatafile = f'{self.basepath}/{scan}/_DM_measurements_data.txt'
            Log = np.genfromtxt(measdatafile, unpack=True, delimiter=' ', skip_header=0, filling_values=0, invalid_raise=False)
            ...
            if (#self.fCND == [] or self.gCND == [] 
                ...
                for runind, file in enumerate(self.allDMfiles[scanindex]):
                    ...
                    if self.singlerunfile is not None: matlogfile = self.singlerunfile 
                    else: matlogfile = rf'{self.basepath}/{scan}/AxionFinder results/{self.analysisname}/{runind+1}/{self.analysisname}_vars.mat'        
                    mat_data = loadmat(matlogfile)
                    try:
                        fCND = mat_data['fCND_new'].flatten().tolist() # all run cand fs
                        gCND = mat_data['gCND_new'].flatten().tolist() # all run cand gs
                    except KeyError:
                ...
                for runind, file in enumerate(self.allDMfiles[scanindex]):
                    ...
                    for candind, candfreq in enumerate(fCND[runind]):
                        if np.abs(candfreq-run_larmor) < candidate_window:
                            self.cands_res.append([scan, runind, fCND[runind][candind], gCND[runind][candind]])
                            #print(f'diff {np.abs(candfreq-run_larmor)} between candfreq {candfreq} and larmor {run_larmor} INSIDE window {candidate_window}')
                        else:
                            self.cands_bkg.append([scan, runind, fCND[runind][candind], gCND[runind][candind]])
            ...
            print(f'keeping only outliers within a {candidate_window}Hz resonance window around larmor freq')
            fvals_res = [cand[2] for cand in cands_filtered if cand[0] == scan]
            gvals_res = [cand[3] for cand in cands_filtered if cand[0] == scan]
            check(fvals_res)
            check(gvals_res)

            if len(plotcolors) < len(scans): plotcolors.append(get_random_color())
            ...
            plt.plot(fvals_res-centerfreq, gvals_res, marker='*', linestyle='None', markersize=30, color=plotcolors[scanindex], label=f'Candidates from scan {scan}')

        # plot a number at each point stating the run where the cand was found
        for cand in cands_filtered: plt.text(cand[2]-centerfreq, cand[3], str(cand[1]), ha='center', va='center', fontsize=15, color='black')
        ...
        plt.autoscale()
        plt.tight_layout()
        if len(scans) > 1: scanstr = 'all'
        else: scanstr = scan 
        if saveplot_opt: plt.savefig(f'{savepath}/AxionFinder_{self.analysisname}_Cands_cut_resonant_{scanstr}.png', bbox_inches='tight')
        if showplot_opt: plt.show()        
        plt.clf()
        plt.close()
        ...
        if cut0:
            ...
            for scanindex, scan in enumerate(scans):
                ...
                check(fvals_merge)
                check(gvals_merge)
                plt.plot(fvals_merge-centerfreq, gvals_merge, marker='*', linestyle='None', markersize=30, color=plotcolors[scanindex], label=f'Candidates from scan {scan}')
            ...
            if saveplot_opt: plt.savefig(f'{self.basepath}/AxionFinder_{self.analysisname}_Cands_cut_merge_{scanstr}.png', bbox_inches='tight')   
            if showplot_opt: plt.show()   
            plt.clf()
            plt.close()
        ...
        if cut1:
            print(f'removing outliers that appear off-resonance in other spectra within {background_cut}Hz')
            cands_to_cut = []
            for candindex, cand_res in enumerate(cands_filtered):
                close_to_cand_within_bkg = [cand_bkg for cand_bkg in self.cands_bkg if np.abs(cand_bkg[2] - cand_res[2]) <= background_cut]
                #check(cand_res)
                #check(close_to_cand_within_bkg)
                if len(close_to_cand_within_bkg) > 0: cands_to_cut.append(cand_res)
            cands_filtered = [cand for cand in cands_filtered if cand not in cands_to_cut]
            ...
            for scanindex, scan in enumerate(scans):
                ...
                check(fvals_bkg)
                check(gvals_bkg)
                plt.plot(fvals_bkg-centerfreq, gvals_bkg, marker='*', linestyle='None', markersize=30, color=plotcolors[scanindex], label=f'Candidates from scan {scan}')
            ...
            if saveplot_opt: plt.savefig(f'{self.basepath}/AxionFinder_{self.analysisname}_Cands_cut_bkg_{scanstr}.png', bbox_inches='tight')
            if showplot_opt: plt.show() 
            plt.clf()
            plt.close()
        ...
        if compare_candidates:
            ...
            def return_spectrum_window(scan, run, centerfreq, width, convolved=False):
                ...
                if convolved == True:
                    ...
                    freqs = mat_data['convolvedFreq'].flatten().tolist() # frequencies array of convolved PSD
                    specs = mat_data['convolvedPeak'].flatten().tolist() # convolved PSD values array
                    powerOfTenScaling = mat_data['powerOfTenScaling'].flatten().tolist()[0]
                ...
                freq_window = [freq for freq in freqs if (centerfreq-width/2) <= freq <= (centerfreq+width/2)]         
                spec_window = [spec for spec in specs if (centerfreq-width/2) <= freqs[specs.index(spec)] <= (centerfreq+width/2)]

                return freq_window, spec_window

            # get all candidates per scan
            rescan_inds = [1,2]
            cands_main = [cands for cands in cands_filtered if cands[0] == mainscan_name]
            num_plots = len(cands_main)
            num_columns = 2
            num_rows = (num_plots + num_columns - 1) // num_columns
            check(num_rows)
            #plt.figure(figsize=(16*0.9, 9*0.9), dpi=100)
            fig, axes = plt.subplots(num_rows, num_columns, figsize=(16*0.9, 9*0.9), dpi=100)
            plt.rc('font', size=10)
            specwidth = 20
            ...
            for i, cand in enumerate(cands_main):
                ...
                freq_windowC, spec_windowC = return_spectrum_window(scan=mainscan_name,run=cand[1],centerfreq=cand[2],width=specwidth)
                print('main spec window retrieved')
                axion_line = axion_lineshape(wind.v_0, wind.v_lab, nu_a=cand[2], nu=np.array(freq_windowC), case="grad_perp", alpha=np.pi / 2) # the axion lineshape (Gramolin paper)
                ...
                nu_n, FWHM_n, amp_n, nu_n_err, FWHM_n_err, amp_n_err = ReturnNMRdata(self.basepath, scan, cand[1])
                year, month, day, time_str, datetime_int = ReturnMeasureTimFromFile(file)#2022, 12, 23, '15:47:18'
                
                fakeAxion_Amp = wind.GetALP_Data(year=year,month=month,day=day,time_str=time_str,datetime_int=datetime_int,
                                                        T_acq=0.5,T2time=ReturnT2time(self.basepath, scan),
                                                        NMRfreq=[nu_n,nu_n_err],
                                                        NMRwidth=[FWHM_n,FWHM_n_err],
                                                        NMRamp=[amp_n,amp_n_err],
                ...
                axes[i].scatter(np.array(freq_windowC)-cand[2], spec_windowC, label=cand[0], color=plotcolors[mainscan_ind])
                axes[i].plot(0, fakeAxion_Amp*(cand[3]**2), marker='*', linestyle='None', markersize=25, color=plotcolors[mainscan_ind])
                axes[i].text(0, fakeAxion_Amp*(cand[3]**2), str(f'{cand[1]}'), ha='center', va='center', fontsize=10, color='black')
                axes[i].plot(np.array(freq_windowC)-cand[2], axion_line, color='red', linestyle='--')
                #axes[i].plot(np.array(conv_freqstamp)-cand[2], conv_PSD, color='red', linestyle='--')

                # scan B
                freq_windowB, spec_windowB = return_spectrum_window(scan=scans[rescan_inds[1]],run=cand[1],centerfreq=cand[2],width=specwidth)
                print('sub-1 spec window retrieved')
                axes[i].scatter(np.array(freq_windowB)-cand[2], spec_windowB, label=scans[rescan_inds[1]], color=plotcolors[rescan_inds[1]])
                cands_B = [candB for candB in cands_filtered if candB[0] == scans[rescan_inds[1]] and candB[2] >= np.min(freq_windowC) and candB[2] <= np.max(freq_windowC)]
                ...
                axes[i].plot(np.array(cands_B_fvals)-cand[2], fakeAxion_Amp*(np.array(cands_B_gvals)**2), marker='*', linestyle='None', markersize=20, color=plotcolors[rescan_inds[1]])        
                for candB in cands_B:
                    axes[i].text(candB[2]-cand[2], fakeAxion_Amp*(np.array(candB[3])**2), str(candB[1]), ha='center', va='center', fontsize=10, color='black')

                # scan A
                freq_windowA, spec_windowA = return_spectrum_window(scan=scans[rescan_inds[0]],run=cand[1],centerfreq=cand[2],width=specwidth)
                print('sub-2 spec window retrieved')
                axes[i].scatter(np.array(freq_windowA)-cand[2], spec_windowA, label=scans[rescan_inds[0]], color=plotcolors[rescan_inds[0]])
                cands_A = [candA for candA in cands_filtered if candA[0] == scans[1] and candA[2] >= np.min(freq_windowC) and candA[2] <= np.max(freq_windowC)]
                cands_A_fvals = [candA[2] for candA in cands_A]
                cands_A_gvals = [candA[3] for candA in cands_A]
                axes[i].plot(np.array(cands_A_fvals)-cand[2], fakeAxion_Amp*(np.array(cands_A_gvals)**2), marker='*', linestyle='None', markersize=20, color=plotcolors[rescan_inds[0]])        
                for candA in cands_A:
                    axes[i].text(candA[2]-cand[2], fakeAxion_Amp*(np.array(candA[3])**2), str(candA[1]), ha='center', va='center', fontsize=10, color='black')

                axes[i].set_xlabel(f"Frequency $-{round(cand[2]*1e-8, 6)}~[MHz]$", fontsize=10)
            ...
            plt.tight_layout()
            if saveplot_opt:
                plt.savefig(f'{self.basepath}/AxionFinder_Cands_Comparison_Raw.png', bbox_inches='tight')
            if showplot_opt:
                plt.show()
            plt.close() 
        ...
        if cut2:
            ...
            if mainscan_name != None:
                ...
                cands_mainscan = [cand for cand in cands_filtered if cand[0] == mainscan_name]
                cands_rescans = [cand for cand in cands_filtered if cand[0] != mainscan_name]

                for mainscan_cand in cands_mainscan:
                    consistent_rescan_cands = [rescan_cand for rescan_cand in cands_rescans if np.abs(rescan_cand[2] - mainscan_cand[2]) <= rescan_cut ]
                ...
                consistent_freqs = find_matching_tuples(cands_sorted_by_scans, margin=rescan_cut) # rescan_cut # consistent set appears at 38Hz margin
                check(consistent_freqs)
                # find the indices of the consistent candidates
                for ntuple in consistent_freqs:
                    cands = []
                    for freq in ntuple:
                        scan = [cand[0] for cand in cands_filtered if cand[2] == freq][0]
                        run = [cand[1] for cand in cands_filtered if cand[2] == freq][0]
                        gval = [cand[3] for cand in cands_filtered if cand[2] == freq][0]
                        cands.append([scan,run,freq,gval])
                    consistent_sets.append(cands)

            print(f'checked for sets of outliers consistent between main- and rescans to within {rescan_cut} Hz')
            ...
            for scanindex, scan in enumerate(scans):
                ...
                plt.plot(thisscan_fvals-centerfreq, thisscan_gvals, marker='*', linestyle='None', markersize=30, color=plotcolors[scanindex], label=f'Candidates from scan {scan}')
            for ntuple in consistent_sets:
                for cand in ntuple:
                    ...
                    plt.text(cand[2]-centerfreq, cand[3], str(f'{cand[0]}-{cand[1]}'), ha='center', va='center', fontsize=15, color='black')
            ...
            if saveplot_opt: plt.savefig(f'{self.basepath}/AxionFinder_{self.analysisname}_Cands_cut_rescan_{scanstr}.png', bbox_inches='tight')
            if showplot_opt: plt.show()  
            plt.clf()
            plt.close()

        consistent_mainscan_cand = [cand for cand_tuple in consistent_sets for cand in cand_tuple if cand[0] == mainscan_name]
    ...
    def GetExclusion(
        ...
        if coupling_unit == "eV": unitfactor = 1e-9
        elif coupling_unit == "MeV": unitfactor = 1e-3         
        elif coupling_unit == "GeV": unitfactor = 1   
        else: print('unknown coupling unit specified')
        ...
        for scanindex, scan in enumerate(scans):
            ...
            measLog = np.genfromtxt(measdatafile, unpack=True, delimiter=' ', skip_header=0, filling_values=0, invalid_raise=False)
            ...
            if load_from_where == 2: # load from previously saved txt files              
                ...
                limitlog = np.genfromtxt(limitfile, unpack=True, delimiter=' ', skip_header=1, filling_values=0, invalid_raise=False)
                ...
                for run, file in enumerate(self.allDMfiles[scanindex]):
                    ...
                    if load_from_where == 0: # load data from mat logfiles
                        ...
                        glim = mat_data['gLIM'].flatten()[0] # calculated coupling for 5sigma signal threshold
                        #check(flim)
                    elif load_from_where == 1: # recalculate now          
                        flim = mat_data['fLIM'].flatten()[0] # center frequency of this run's analysis window

                        if self.singlerunfile is not None:
                            nu_n, FWHM_n, amp_n, nu_n_err, FWHM_n_err, amp_n_err = ReturnNMRdata(self.basepath, scan, self.singlerunindex)
                            year, month, day, time_str, datetime_int = ReturnMeasureTimFromFile(rf'{self.basepath}/{scan}/AxionScan_{self.singlerunindex}.h5')
                            AxionParams = mat_data['fakeAxionParameters'].flatten().tolist()
                            fakeAxion_Amp = AxionParams[5]
                        else:
                            nu_n, FWHM_n, amp_n, nu_n_err, FWHM_n_err, amp_n_err = ReturnNMRdata(self.basepath, scan, run)
                            year, month, day, time_str, datetime_int = ReturnMeasureTimFromFile(file)#2022, 12, 23, '15:47:18'
                            fakeAxion_Amp = AxionWind().GetALP_Data(year=year,month=month,day=day,time_str=time_str,datetime_int=datetime_int,
                                                                    T_acq=0.5,T2time=ReturnT2time(self.basepath, scan),
                                                                    NMRfreq=[nu_n,nu_n_err],
                        ...
                        std_convPSD_mat = mat_data['convolvedPSDstd'].flatten()[0] * (10**self.powerOfTenScaling) # don't use this, it's nonsense if the matlab fit fails
                        check(std_convPSD_mat)
                        std_convPSD = np.std(mat_data['convolvedPeak'].flatten().tolist()) * (10**self.powerOfTenScaling)
                        check(std_convPSD)
                        glim = np.sqrt( 5.0 * std_convPSD / fakeAxion_Amp)
                        check(glim)
                        if np.abs(glim) > 5*np.mean(glims) and len(glims) > 1: glim = np.mean(glims) # something went wrong
                    ...
                    if limit_errs:              
                        ...
                        ax_data = AxionWind().GetALP_Data(year=year,month=month,day=day,time_str=time_str,datetime_int=datetime_int,
                                                                T_acq=0.5,T2time=ReturnT2time(self.basepath, scan),
                                                                NMRfreq=[nu_n,nu_n_err],
                                                                NMRwidth=[FWHM_n,FWHM_n_err],
                                                                NMRamp=[amp_n,amp_n_err],
                        ...
                        glim_err = np.sqrt(
                            (dglim_dstd * std_err)**2 +
                            (dglim_dAmp * ax_amp_err)**2
                        )
                    ...
                    print_progress_bar(run, total=len(self.allDMfiles[scanindex]), prefix='Progress', suffix='Complete', length=50)

                np.savetxt(rf'{savepath}/exclusion_limits_{scan}.txt', \
                        np.column_stack((flims, glims, glims_errs)), header='Frequency (Hz) & g_aNN_lim (GeV^-1) & Delta g_aNN_lim')
                check(glims)

        #check(coupling_limits)
        glims_mainscan = [limit[3] for limit in coupling_limits if limit[0] == mainscan_name]
        ...
        if merge_limits:
            ...
            if load_from_where == 1:
                for index in range(len(coupling_limits[0])):
                    top = (coupling_limits[0][index]/glims_errs[0][index]**2) + (coupling_limits[1][index]/glims_errs[1][index]**2) + (coupling_limits[2][index]/glims_errs[2][index]**2)
                    down = (1/glims_errs[0][index]**2) + (1/glims_errs[1][index]**2) + (1/glims_errs[2][index]**2)
                    glims_merged_meas.append(top/down)
                    glims_merged_err.append(down**(-1/2))
                    glims_merged_SNR.append(np.sqrt( 5 / np.sqrt( (5/coupling_limits[0][index]**2)**2 + (5/coupling_limits[1][index]**2)**2 + (5/coupling_limits[2][index]**2)**2 ) ))
                    glims_merged_prop.append( (1/( 1/coupling_limits[0][index]**4 + 1/coupling_limits[1][index]**4 + 1/coupling_limits[2][index]**4 ))**(1/4) )

                np.savetxt(rf'{savepath}/exclusion_limt_combined.txt', \
                            np.column_stack((flims, glims_merged_prop)), header='Frequency (Hz) g_aNN_lim (GeV^-1)')
                
            elif load_from_where == 2:
                limitcombfile = rf"{savepath}/exclusion_limt_combined.txt"
                limitcomblog = np.genfromtxt(limitcombfile, unpack=True, delimiter=' ', skip_header=1, filling_values=0, invalid_raise=False)
        ...
        if corr_rescans and len(scans) > 1 and candidates_log is not None:
            ...
            couplings_rescan = []
            for scan in [scan for scan in scans if scan != mainscan_name]: # all rescans
                couplings_rescan.append([[limit[0], limit[1], limit[2], limit[3]] for limit in coupling_limits if limit[0] == scan])
            ...
            for index, i in enumerate(freq):
                candidates_list.append([scan[index], run[index], freq[index], gval[index]])
            #check(candidates_list)

            for cand_runindex in list({cand[1] for cand in candidates_list if cand[0] == mainscan_name}): # all runs in the main scan where candidates were found
                limits_rescan = []
                for rescan in couplings_rescan:
                    limits_rescan.append([coupling[3] for coupling in rescan if coupling[1] == cand_runindex])
                glims_mainscan[cand_runindex] = np.mean(limits_rescan)

        # save the (corrected) main scan sensitivity as a list
        ALP_masses = [limit[2]*AxionWind().h_eV for limit in coupling_limits if limit[0] == mainscan_name]
        #check(ALP_masses)
        check(glims_mainscan)
        np.savetxt(rf'{savepath}/exclusion_limits_main.txt', \
                np.column_stack((ALP_masses, glims_mainscan)), header='CASPEr-gradient low-field\n m_a [eV]  g_an/m_n [GeV^-1]')
        ...
        if saveplot_opt:
            ...
            for scanindex, scan in enumerate(scans):
                flims_thisscan = [limit[2] for limit in coupling_limits if limit[0] == scan]
                glims_thisscan = [limit[3] for limit in coupling_limits if limit[0] == scan]
                gerrs_thisscan = [limit[4] for limit in coupling_limits if limit[0] == scan]
                ...
                ax.errorbar(np.array(flims_thisscan)-centerfreqs[scanindex], np.array(glims_thisscan)*unitfactor, yerr=gerrs_thisscan, fmt='o', color=plotcolors[scanindex], linestyle='-', label=f"{scan_labels[scanindex]}")
            ...
            while scan_ind >= 0:
                flims_thisscan = [limit[2] for limit in coupling_limits if limit[0] == scans[scan_ind]]
                glims_thisscan = [limit[3] for limit in coupling_limits if limit[0] == scans[scan_ind]]
                ax.fill_between(np.array(flims_thisscan)-centerfreqs[scan_ind], np.array(glims_thisscan)*unitfactor, max, color=plotcolors[scan_ind], alpha=0.3)
                scan_ind -= 1
            ...
            plt.xlabel(rf"Frequency, $\nu_a$-{1348450} [Hz]")
            plt.ylabel(rf"$|g_{{\mathrm{{ap}}}}|~[\mathrm{{GeV}}^{-1}]$") #~[{coupling_unit}^{-1}]$") # g_{{\mathrm{{aNN}}}}
            plt.legend(loc='best')
            #plt.grid()
            #plt.autoscale()
            plt.tight_layout()
            if use_modulation: plt.savefig(rf'{savepath}/limitsplot.png', bbox_inches='tight')
            else: plt.savefig(rf'{savepath}/limitsplot_no_modulation.png', bbox_inches='tight')
            if showplot_opt: plt.show()  
            plt.clf()
    ...
    def GetFakeAxionRecovery(
        ...
        if scan == "" or scan == None:
            ...
            if isinstance(scan, str): scan = [scan]    
                #print('single scan input')              
            elif isinstance(scan, list) and all(isinstance(i, str) for i in scan): scan = scan
                #print('list of scans input')
            else: raise OSError("no valid scan inputted. must be string or list of strings.")
                
        for subscan in scan:
            if compare_SGstepsize:
                scanpath = rf"{self.basepath}/{subscan}/AxionFinder results/fakeAxionScan1"
                SGstepsizes = []
            else:
                scanpath = rf"{self.basepath}/{subscan}/AxionFinder results/{mat_analysisname}"
            check(scanpath)
            all_subfolders = [f for f in glob.glob(os.path.join(scanpath, '*/')) if os.path.isdir(f)]
            run_directories = [os.path.basename(folder[:-1]) for folder in all_subfolders if "fakeAxion" in os.path.basename(folder[:-1])]
            #if all(not run_directory for run_directory in run_directories):
            run_directories = run_directories + [os.path.basename(folder[:-1]) for folder in all_subfolders if "AxionScan" in os.path.basename(folder[:-1])] 
            ...
            for run_num, run in enumerate(run_directories):
                ...
                if check_cands:
                    ...
                    gcands = mat_data['gCND'].flatten().tolist() # all run cand couplings
                    ...
                    if len(cands_thisrun) < 1:
                        ...
                        if identify_cand_by == 1:
                            closestcand_ind = min(range(len(fcands)), key=lambda i: abs(fcands[i] - self.fakeAx_fIN[run_num]))
                            ...
                            #check(axionpeakfreq)
                            closestcand_ind = min(range(len(fcands)), key=lambda i: abs(fcands[i] - axionpeakfreq))
                        elif identify_cand_by == 3:
                            #closestcand_ind = np.argmin([abs(self.fakeAx_fIN[run_num] - gcand) for gcand in gcands])
                            closestcand_ind = min(range(len(gcands)), key=lambda i: abs(gcands[i] - self.fakeAx_gIN[run_num]))

                        truecand = cands_thisrun[closestcand_ind]
                        print(f'injected axion frequency: {self.fakeAx_fIN[run_num]}')
                        print(f'closest cand found within {abs(self.fakeAx_fIN[run_num] - fcands[closestcand_ind])} Hz at: {truecand}')
                    ...
                    try:
                        ...
                        gout = mat_data['fakeAxion_convolvedCoupling'].flatten().tolist()[0]                        
                    
                    if gout.real == 0 and gout.imag != 0: # this happens if the value of the convolution is negative
                ...
                self.fakeAx_frecovery.append([self.fakeAx_fIN[run_num],self.fakeAx_fOUT[run_num]])
                self.fakeAx_grecovery.append([self.fakeAx_gIN[run_num],self.fakeAx_gOUT[run_num]])     
                #check(self.fakeAx_frecovery)
                #check(self.fakeAx_grecovery)

            print_progress_bar(run_num, total=len(run_directories), prefix='Progress', suffix='Complete', length=50)
            ...
            if not is_stoc_signal:
                smoothaxdata = pd.DataFrame(self.fakeAx_grecovery, columns=["g_in", "g_out"])
                if is_SG_scan:
                    smoothaxdata.to_excel(rf"{savepath}/fakeAxionScanTest_SG_smooth.xlsx", index=False)
                else:
                    smoothaxdata.to_excel(rf"{savepath}/fakeAxionScanTest_smooth.xlsx", index=False)
            else:
                if is_SG_scan:
                    smoothaxdata = pd.read_excel(rf"{savepath}/fakeAxionScanTest_SG_smooth.xlsx")
                else:
                    smoothaxdata = pd.read_excel(rf"{savepath}/fakeAxionScanTest_smooth.xlsx")
            ...
            if compare_SGstepsize:
                ...
                for stepsize in SGstepsizes:
                    f_IN = [cand[0] for cand in self.fakeAx_fIN if cand[2] == stepsize[1]][0]
                    f_OUT = [cand[0] for cand in self.fakeAx_fOUT if cand[2] == stepsize[1]][0]
                    g_IN = [cand[1] for cand in self.fakeAx_fIN if cand[2] == stepsize[1]][1]
                    g_OUT = [cand[1] for cand in self.fakeAx_fOUT if cand[2] == stepsize[1]][1]
                ...
                ax1.scatter(stepsizes, f_diffs, s=180, color='blue', alpha=0.9, linewidth=1)
                ...
                ax2 = ax1.twinx()
                ax2.scatter(stepsizes, g_diffs, s=180, color='green', alpha=0.9, linewidth=1)
                ...
                #plt.autoscale()
                plt.tight_layout()
                plt.title(rf'Fake ALP $f={self.fakeAx_fIN[0][0]}$, $g={self.fakeAx_gIN[0][1]}$')
                plt.savefig(rf'{savepath}/FakeAxion_SGstepsizes.png', bbox_inches='tight')
                plt.show()
                plt.clf()
                ...
                first_jump = find_first_differing_element2(self.fakeAx_gOUT,self.fakeAx_gIN,diff_thresh)
                ...
                gvals_x = np.array([gval[0] for gval in self.fakeAx_grecovery if gval[0] >= cutoff_lo and gval[0] <= cutoff_hi])
                gvals_y = np.array([gval[1] for gval in self.fakeAx_grecovery if gval[0] >= cutoff_lo and gval[0] <= cutoff_hi])
                fvals_x = np.array([fval[0] for index, fval in enumerate(self.fakeAx_frecovery) if self.fakeAx_grecovery[index][0] >= cutoff_lo])
                fvals_y = np.array([fval[1] for index, fval in enumerate(self.fakeAx_frecovery) if self.fakeAx_grecovery[index][0] >= cutoff_lo])
                ...
                if is_stoc_signal:
                    ...
                    first_jump = find_first_differing_element2(smooth_gOUT,smooth_gIN,diff_thresh)
                    #cutoff_smooth = smooth_gIN[smooth_gOUT.index(first_jump)]
                    cutoff_smooth = 0.030
                    smooth_x = np.array([gval[0] for gval in fakeAx_grecovery_smooth if gval[0] >= cutoff_smooth and gval[0] <= cutoff_hi])
                    smooth_y = np.array([gval[1] for gval in fakeAx_grecovery_smooth if gval[0] >= cutoff_smooth and gval[0] <= cutoff_hi])
                ...
                if plotopt == 0:
                    ...
                    plt.scatter(self.fakeAx_gIN, self.fakeAx_gOUT, color='green', alpha=0.9, linewidth=1)
                    plt.plot(gvals_x, fitfunc(gvals_x, *popt_fuzzy), color='green', alpha=0.9, linewidth=1, label=r"Analysis bias: ${:.3f}$".format(popt_fuzzy[0]))
                    plt.axvline(x=cutoff_lo, color='green', linestyle='--', linewidth=4, alpha=0.5, label=r"Detection cutoff: {:.3f} $\mathrm{{GeV}}^{{-1}}$".format(cutoff_lo))

                    if is_stoc_signal:
                        plt.scatter(smooth_gIN, smooth_gOUT, color='red', alpha=0.9, linewidth=1)
                        plt.plot(smooth_x, fitfunc(smooth_x, *popt_smooth), color='red', alpha=0.9, linewidth=1, label=r"Analysis bias: ${:.3f}$".format(popt_smooth[0]))
                        plt.axvline(x=cutoff_smooth, color='pink', linestyle='--', linewidth=4, alpha=0.5, label=r"Detection cutoff: {:.3f} $\mathrm{{GeV}}^{{-1}}$".format(cutoff_smooth))
                    ...
                    axdata = pd.read_excel(f'{self.basepath}/{subscan}/AxionFinder results/{psdplotpath}')
                    freqs = axdata.iloc[:, 0].tolist()
                    specs = axdata.iloc[:, 1].tolist()
                    smooth_axline = axdata.iloc[:, 2].tolist()
                    try:
                        random_axline = axdata.iloc[:, 3].tolist()
                    except IndexError: # if non-stochastic axions have been injected, this column will be empty
                    ...
                    axs[0].scatter(np.array(freqs), np.array(specs), label='Flux PSD', color='blue', zorder=0, alpha=0.8, linewidth=0.2)
                    axs[0].plot(np.array(freqs), (np.mean(np.array(specs)-random_axline)+random_axline), color='green', alpha=0.9, linewidth=1, zorder=1, label='Stochastic ALP')                  
                    axs[0].plot(np.array(freqs), (np.mean(np.array(specs)-smooth_axline)+smooth_axline), color='red', alpha=0.9, linewidth=2, zorder=2, label='Averaged ALP')
                    axs[0].set_xlabel(specxlabel)#, fontsize=20
                    axs[0].set_ylabel(rf'PSD [{specyunit}]')
                    axs[0].tick_params(axis='both', which='major')#, labelsize=20
                    axs[0].set_xlim(98,108)
                    axs[0].set_ylim(1e-12, np.max(specs)*(1+1e-1)) #min 1e10*np.min(specs)
                    ...
                    if is_stoc_signal:
                        if not is_SG_scan:
                            ax_avg_label="Averaged axion, SG off: ${:.4f}$".format(popt_smooth[0])
                            ax_stoc_label="Stochastic axion, SG off: ${:.4f}$".format(popt_fuzzy[0])
                        else:
                            ax_avg_label="Averaged axion: ${:.4f}$".format(popt_smooth[0])
                            ax_stoc_label="Stochastic axion: ${:.4f}$".format(popt_fuzzy[0])
                            check(popt_fuzzy[0])
                        ...
                        if not is_SG_scan:
                            ax_stoc_label="Averaged axion, SG off: ${:.4f}$".format(popt_fuzzy[0])
                        else:
                            ax_stoc_label="Averaged axion: ${:.4f}$".format(popt_fuzzy[0])
                    ax_stoc_linelabel="" #r"Detection cutoff: {:.3f} $\mathrm{{GeV}}^{{-1}}$".format(cutoff_lo)
                    axs[1].scatter([gval[0] for gval in self.fakeAx_grecovery if gval[0] <= cutoff_hi], [gval[1] for gval in self.fakeAx_grecovery if gval[0] <= cutoff_hi], color='green', alpha=0.8, linewidth=1)
                    axs[1].plot(gvals_x, fitfunc(gvals_x, *popt_fuzzy), color='green', alpha=0.9, linewidth=1)#label=ax_stoc_label
                    axs[1].axvline(x=cutoff_lo, color='green', linestyle='--', linewidth=2, alpha=0.4)#label=ax_stoc_linelabel
                    ...
                    axs[0].text(-0.1, 1.05, "(a)", transform=axs[0].transAxes, va='top') # fontweight='bold'
                    ...
                    # (top-left)
                    axs[0, 0].scatter(gvals_x, gvals_y, color='green', alpha=0.9, linewidth=1)
                    axs[0, 0].plot(gvals_x, fitfunc(gvals_x, *popt_fuzzy), color='green', alpha=0.9, linewidth=1, label=r"Analysis bias: ${:.3f}$".format(popt_fuzzy[0]))
                    ...
                    axs[0, 0].set_xlabel(r"coupling IN [$\mathrm{GeV}^{-1}$]", fontsize=14)
                    axs[0, 0].set_ylabel(r"coupling OUT [$\mathrm{GeV}^{-1}$]", fontsize=14)
                    axs[0, 0].tick_params(axis='both', which='major', labelsize=12)
                    axs[0, 0].legend(fontsize=14)

                    # (top-right)
                    axs[0, 1].scatter(gvals_x, fvals_y*1e-6, color='orange', alpha=0.9, linewidth=1)
                    axs[0, 1].set_xlabel(r"coupling IN [$\mathrm{GeV}^{-1}$]", fontsize=14)
                    axs[0, 1].set_ylabel(r"frequency OUT [MHz]", fontsize=14)
                    axs[0, 1].tick_params(axis='both', which='major', labelsize=12)

                    # (bottom-left)
                    axs[1, 0].scatter(fvals_x*1e-6, gvals_y, color='pink', alpha=0.9, linewidth=1)
                    axs[1, 0].set_xlabel(r"frequency IN [MHz]", fontsize=14)
                    axs[1, 0].set_ylabel(r"coupling OUT [$\mathrm{GeV}^{-1}$]", fontsize=14)
                    axs[1, 0].tick_params(axis='both', which='major', labelsize=12)

                    # (bottom-right)
                    popt_f, pcov_f = curve_fit(fitfunc, fvals_x*1e-6, fvals_y*1e-6,)
                    axs[1, 1].scatter(fvals_x*1e-6, fvals_y*1e-6, color='blue', alpha=0.9, linewidth=1)
                    axs[1, 1].plot(fvals_x*1e-6, fitfunc(fvals_x*1e-6, *popt_f), color='green', alpha=0.9, linewidth=1, label=r"Frequency accuracy: ${:.3f}$".format(popt_f[0]))
                ...
                plt.tight_layout()
                #plt.grid()
                plt.savefig(rf'{savepath}/{subscan}_{mat_analysisname}_{plottype}_{cands_str}.png', bbox_inches='tight', dpi=400)
...
Mainz = Station(
    ...
    longitude_deg=(8. + 16./60. + 26.2056/3600),  # in [deg]
    elevation=130.,
    verbose=False
)
...
class AxionWind:
    ...
    def __init__(
        ...
        v_0 =  220. * 10**3,  # [m/s] # Local (@ solar radius) galaxy circular rotation speed (in m/s)
        v_lab = 233. * 10**3,  # [m/s] # Laboratory speed relative to the galactic rest frame (in m/s)
        windangle = None,
        # dark matter axion density
        # PDG 2023: 0.3
        # Derek's book: 0.4, also used by Dani
        # SHM+: 0.55    
        rho_DM = (0.55*1e9)*(1e6),  # eV/m^3 !!!! unit changed from cm^3 to m^3  #### 3.6e-42 GeV^4 = 3.6e-6 eV^4 # what CASPEr-e uses
        verbose=False,
    ):
        """
        ...
        """
        self.name = name
        self.c = 299792458.  # [m/s]. speed of light in vacuum
        self.v_0 = v_0
        self.v_lab = v_lab
    ...
    def GetAxionWind(
        ...
        """
        if verbose: print("now calculating wind angle")
        ...
        if self.timeastro is None:
            if (year or month or day or time_hms) is None:
                ...
                print(f"no date and time input provided, using current date and time: {time_DMmeasure}")
            else:
                time_DMmeasure = rf"{year}-{month}-{day}T{time_hms}"
            if verbose: print(f"time input: {time_DMmeasure}")
        ...
        if self.station is None:
            self.station=Mainz
            if verbose: print('no station specified, defaulting to CASPEr Mainz')
        ...
        if verbose: print("time of DM measurement (fractional days): ", DMtimefrac)

        LABvel = wind.ACalcV(DMtimefrac)
        if verbose: print("velocity (lab frame) @DM time: ", LABvel)
        ...
        Bz = unit_Up.get_xyz() # our leading field is pointing up perpendicular to earth's surface
    ...
    def axion_beta(
        ...
        if nu <= nu_a:
            ...
            return (2 * self.c * self.v_lab) / self.v_0**2 * np.sqrt(2*(nu - nu_a)/nu_a)
    ...
    def axion_lambda(
        ...
        p1 = np.exp(- self.axion_beta(nu_a=nu_a, nu=nu)**2 * self.v_0**2 / (4 * self.v_lab**2) - \
    ...
    def axion_C_para(
        ...
        return self.v_0**2 / 2. + self.v_lab**2 * np.cos(alpha)**2
    ...
    def lineshape_t(self,  nu, nu_a=None, case="grad_perp")->np.ndarray:
        ...
        self.GetAxionWind(\
            year=self.year, month=self.month, day=self.day, time_hms=self.time_hms, \
                latitude=self.station.latitude_deg, \
                    longitude=self.station.longitude_deg, 
                        elevation=self.station.elevation, \
                            verbose=False)
        
        return axion_lineshape(v_0=self.v_0, v_lab=self.v_lab, nu_a=nu_a, \
                               nu=nu, case=case, alpha=self.windangle) # type: ignore
    
    #def GetALPamplitude(self, nu_ALP, g_aNN, DMfiles, filepath, index, usedailymod=True):
    def GetALP_Data(self, T_acq, T2time, NMRfreq, NMRwidth, NMRamp, nu_ALP, g_aNN, 
                    year=None, month=None, day=None, time_str=None, datetime_int=None, usedailymod=True, returnerrors=False):
        ...
        if datetime_int is None:
            datetime_int=GetDateTimeSimple(year,month,day,time_str,return_as_int=True)
        ...
        FWHM_ALP = nu_ALP * (self.v_0**2 + self.v_lab**2 * np.sin(self.windangle)**2) / self.c**2 # this is the same as nu_a*b2 in MATLAB
        ...
        V90 = np.sqrt(4 * amp_n * T_acq / T2star) # SQUID readout voltage after a pi/2 pulse
        ...
        rabi = 1/2 * g_aNN * np.sqrt(2*self.hbar_eV*self.c*self.rho_DM) * v_ALP_perp
        ...
        if returnerrors:
            ...
            tau_err = np.abs( -self.c**2 / (np.pi*nu_n**2 * ( self.v_0**2 + self.v_lab**2 * np.sin(self.windangle)**2 )) )

            tpuls_err = (tpuls_is - tpuls_90) * (1+0.05) # add some 5% for fit uncertainty estimation
            ...
            # SQUIDpickup error
            dSQUIDpickup_dtpuls = (np.pi*phi90 * 1/math.tan(np.pi*tpuls_is/2/tpuls_90) * 1/math.sin(np.pi*tpuls_is/2/tpuls_90)) / (2*M0*tpuls_90)
            dSQUIDpickup_dphi90 = 1/math.sin(np.pi*tpuls_is/2/tpuls_90) / M0
            SQUIDpickup_err = np.sqrt( (dSQUIDpickup_dtpuls * tpuls_err)**2 + (dSQUIDpickup_dphi90 * phi90_err)**2 )
            ...
            ds_dtau_ALP = -1 / (np.pi * tau_ALP**2) * (1 / (c + b) - (a + b) / (c + b)**2)
            ...
            # axionamp error
            daxionamp_dpickup = (M0*rabi*spinfraction*T2)**2*tau_ALP*SQUIDpickup / (tau_ALP + T2)
            daxionamp_dspinfraction = (M0*rabi*SQUIDpickup*T2)**2*tau_ALP*spinfraction / (tau_ALP + T2)
            #daxionamp_drabi = (M0*spinfraction*SQUIDpickup*T2)**2*tau_ALP*rabi / (tau_ALP + T2)
            daxionamp_dT2 = (M0*rabi*spinfraction*SQUIDpickup)**2 * tau_ALP*T2 * (2*tau_ALP+T2) / (2*(tau_ALP + T2)**2)
            daxionamp_dtau_ALP = (M0*rabi*SQUIDpickup*spinfraction)**2 * T2**3 / (2*(tau_ALP + T2)**2)
            ...
            errors = [tau_err, T2_err, SQUIDpickup_err, nu_n_err, FWHM_n_err, amp_n_err, axionamp_err]

        return datetime_int, self.v_lab, self.windangle, tau_ALP, T2, nu_n, FWHM_n, amp_n, phi90, axionamp, errors
    ...
    def GetFFTsignal(
        ...
        if rand_amp:
            rvs_amp = expon.rvs(loc=0., scale=1., size=len(freq))
...
def amp_ga_1iter(
    ...
    # step.nps.GetSpectrum(showtimedomain=False, showfreqdomain=True, spectype='PSD')
    step.MovAvgByStep(
        step_len=max(1, step.axion.lw_p // 10),
        verbose=False)
    # step.nps.GetSpectrum(showtimedomain=False, showfreqdomain=True, spectype='PSD')
    r1 = step.nps.Hz2Index(step.axion.nu_a - 5.*step.axion.lw_Hz)
    # check(r1)
...
class SensiCompar:
    def __init__(self,
        ...
        self.gaNNexc = None
        del frequencies, discovery_threshold, detuning_correction, gr, gDM, Q, sigma, Phipito2, T
    ...
    def UpdateExc(
        ...
        if freq_arr is None or PSD is None or measurementT is None or sigma is None:
            ...
            self.gaNNexc = np.sqrt(self.discovery_threshold * self.sigma) * \
                self.Teff_arr ** (-0.25) * (self.Q / self.frequencies) ** (-1.25) * \
        ...
        PSDcorrection_val = ( T2star/(4 * acqtime) * \
            (1 - np.exp(-2 * acqtime / T2star )) * np.exp(-2 * acqdelay / T2star ) \
            ) ** (-1.)
        ...
        PSDinterp = np.interp(self.frequencies[istart:istop+1], freq_arr, PSD_corrected)
        
        # self.Teff_arr 
        Phipito2_new = np.sqrt(PSDinterp* 2)  #  abs(self.frequencies[istart:istop+1] * 2. / self.Q)
        # check(np.mean(Phipito2_new))


        self.Teff_arr[istart:istop+1] += measurementT * \
            (np.sqrt(sigma) / Phipito2_new) ** (-4.) / (np.sqrt(self.sigma) / self.Phipito2) ** (-4.)
    ...
    def PlotExc(
        ...
        ax.plot(hbar / c ** 2 * self.frequencies, self.gaNNexc, label='', color=[0/256., 88/256., 155/256.], alpha=1)  # 0, 88, 155
    ...
    def PlotExc_1PSD(
        ...
        freqoffset=0,
        PSDfreq_range=[-80, 80],
        specxaxis=None, spectrum=None, specxunit=None, specyunit=None,
        ...
        if PSDfreq_range is None or specxaxis is None or spectrum is None or specxunit is None or specyunit is None: 
            raise ValueError('PSDfreq_range is None specxaxis is None or spectrum is None or specxunit is None or specyunit is None')   
        else: 
        ...
        # fig.subplots_adjust(left=left_spc, top=top_spc, right=right_spc,
        #                     bottom=bottom_spc, wspace=xgrid_spc, hspace=ygrid_spc)
        PSD_ax.plot(specxaxis - freqoffset, spectrum, label='PSD ', color=[0/256., 88/256., 155/256.], alpha=1)
        ...
        if freqoffset == 0:
            ...
            PSD_ax.set_xlabel(f'Frequency - {freqoffset:.0f} / ' + specxunit)  # $\\mathrm{Hz}$
        ...
        ax.plot(2 * np.pi * hbar / c ** 2 * massfactor * (self.frequencies - freqoffset), \
            self.gaNNexc, \
            label='gaNN', color=np.array([193, 0, 42])/256., alpha=1)  # 0, 88, 155
        # ax.scatter(GammaandSAmp_arr, GammaandSAmp_arr, marker='x', s=30, color='tab:blue', alpha=1)
        ax.set_xlabel(f'ALP mass - {hbar / c ** 2 * freqoffset * massfactor:.0f}' + \
        ...
        # ax.set_yticks([])
        ax.set_xlim(2 * np.pi * hbar / c ** 2 * massfactor * (freq_start), \
            2 * np.pi * hbar / c ** 2 * massfactor * (freq_stop))
        ax.set_ylim(top=10**(-1.8), bottom=10**(-6.2))
        if ax_yticks is not None:
            ax.set_yticks(ax_yticks)
        ax.fill_between(2 * np.pi * hbar / c ** 2 * massfactor * (self.frequencies - freqoffset), \
            self.gaNNexc, 1e5, color = 'r', alpha=0.2)
        ...
        for i, axi in enumerate([PSD_ax]):
            ...
            axi.text(x=xleft, y=ytop, s = letters[i], ha='right', va = 'center', color='blue', fontsize=14)
        for i, axi in enumerate([ax]):
            xleft, xright = axi.get_xlim()
            ybottom, ytop = axi.get_ylim()
            axi.text(x=xleft, y=ytop, s = letters[i+1], ha='right', va = 'top', color='blue', fontsize=14)
        plt.tight_layout()
        plt.show()
        return self.frequencies, self.gaNNexc
