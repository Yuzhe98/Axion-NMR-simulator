...
from SimuTools import TestSample10MHzT  #Xe129, Methanol, , Mainz, TestStation, MagVec, AxionWind
...
gs = gridspec.GridSpec(nrows=5, ncols=2, height_ratios = [1,1,1,0.1,1.4])  # , width_ratios = [1.5, 1.5], height_ratios = [1,1,1,1]
...
for i, ax in enumerate([ax0, ax2, ax4]):
    ...
    ax.plot(timestamp[0:-1:plotintv], 1e13 * BALP[0:-1:plotintv, 0], label='', color=BALPcolor, alpha=1, lw=0.8)
    ...
    ax_twinx = ax.twinx()
    if i == 0:
        ax_twinx.plot(timestamp[0:-1:plotintv], 1e1 * 1e5 * trjry[1:-1:plotintv, 1], label='', color=trjrycolor, alpha=1)
        ax_twinx.text(x=3.5, y=.8, s='$\\times 10$', fontsize=10, color=trjrycolor)
    else:
...
for i, ax in enumerate([ax1, ax3, ax5]):
    ...
    # ax.plot(GammaandSAmp_arr[:, 0], GammaandSAmp_arr, label='PSD Signal Amp', color='tab:cyan', alpha=1)
    ax.plot(timestamp[0:-1:plotintv], 1e13 * BALP[0:-1:plotintv, 0], label='', color=BALPcolor, alpha=1, lw=0.8)  # darkorange
    # ax.plot(timestamp[0:-1:plotintv], trjry[1:-1:plotintv, 1], label='', color='red', alpha=1)
    ...
    if i == 0:
        ax_twinx.plot(timestamp[0:-1:plotintv], 1e1 * 1e5 * trjry[1:-1:plotintv, 1], label='', color=trjrycolor, alpha=1)
        ax_twinx.text(x=4, y=.8, s='$\\times 10$', fontsize=10, color=trjrycolor)
    else:
        ax_twinx.plot(timestamp[0:-1:plotintv], 1e5 * trjry[1:-1:plotintv, 1], label='', color=trjrycolor, alpha=1)
...
# axempty = fig.add_subplot(gs[3,:])
    ...
    ax6.plot(T2star_arr_0825, np.pi * T2star_arr_0825 / 2, \
        label='$\\tau_s N_{eff}/N_{all} = \\pi T_2^*/2$', color='tab:cyan', alpha=1, linestyle='--')
    ax6.hlines(y=taua, xmin = 1e-5, xmax = 3.8e3, colors='darkred', linestyles='dotted', \
    ...
    marker='x', s=40, color='tab:green', alpha=1)# \\langle \\rangle
